      ...
      <div class="bg-white rounded-xl shadow p-4 overflow-auto">
        <table class="min-w-full table-auto border border-gray-300">
          ...
          <tbody id="matkulTableBody">
            @foreach ($matkul as $index => $mtl)
              <tr class="hover:bg-gray-50">
                <td class="py-2 px-4 border border-gray-300 text-center">{{ $index + 1 }}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $mtl['kode_matkul'] }}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $mtl['nama_matkul'] }}</td>
                <td class="py-2 px-4 border border-gray-300">{{ $mtl['sks'] }}</td>
                <td class="py-2 px-4 border border-gray-300 text-center">
                  <a href="{{ route('matkul.edit', $mtl['kode_matkul']) }}" class="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600">Edit</a>
                  <form action="{{ route('matkul.destroy', $mtl['kode_matkul']) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                    @csrf
                    @method('DELETE')
